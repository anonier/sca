...
@Slf4j
@Component
public class DingTalkQrcodeAuthenticationProvider implements AuthenticationProvider, MessageSourceAware {
    ...
    public Authentication authenticate(Authentication authentication) throws AuthenticationException {
        ...
        if (!"dingTalk_qrcode".equals(grant_type)) {
            return null;
        }
        ...
        if (Boolean.FALSE.equals(stringRedisTemplate.hasKey(phone + "_" + "qrcode")) || !code.equals(stringRedisTemplate.opsForValue().get(phone + "_" + "qrcode"))) {
        ...
        //清除redis中的短信验证码
        stringRedisTemplate.delete(phone + "_" + "qrcode");
        UserDetails user;
        try {
        ...
        check(user);
        DingTalkQrcodeAuthenticationToken authenticationToken = new DingTalkQrcodeAuthenticationToken(user, code, user.getAuthorities());
    ...
    public boolean supports(Class<?> aClass) {
        return DingTalkQrcodeAuthenticationToken.class.isAssignableFrom(aClass);
    }
